...
//					System.out.println("Edge e" + " = new Edge(false, " + "d" + indexOfStartDot + ", " + dots.indexOf(endDot) + ");");
    ...
    public Node minimax(boolean isMaximizing, Node node, int depth, int[] nodeCountRef) {
...
//                nodeCount++;
                ...
                if (child.isHasNewSquare()) {
                    ...
                    evaluatedNode = minimax(true, child, depth - 1,nodeCountRef);
                } else {
                    evaluatedNode = minimax(false, child, depth - 1, nodeCountRef);
                }
...
//            List<Node> children = getChildrenOfNode(isMaximizing, node);
            ...
            for (Node child : children) {
//                nodeCount++;  // Tăng số node đã duyệt
                nodeCountRef[0]++;
                Node evaluatedNode;
                if (child.isHasNewSquare()) {
                    // Nếu tạo ô vuông thì lượt tiếp theo không đổi
        ...
        long executionTime = endTime - startTime;  // Tính thời gian thực thi cho lượt tìm kiếm hiện tại

        System.out.println("Minimax - Thời gian chạy: " + executionTime + " milliseconds");
//        System.out.println("Minimax - Số node đã duyệt: " + nodeCount + " node");
    ...
    public Node minimaxAB(boolean isMaximizing, Node node, int depth, int alpha, int beta, int[] nodeCountRef) {
        long startTime = System.currentTimeMillis();  // Lấy thời gian bắt đầu
        Node bestNode = null;
//        int nodeCount = 0;
...
//                nodeCount++;
                ...
                if (child.isHasNewSquare()) {
                    // Nếu tạo ô vuông thì lượt tiếp theo không đổi
                    evaluatedNode = minimaxAB(true, child, depth - 1, alpha, beta, nodeCountRef);
                } else {
                    evaluatedNode = minimaxAB(false, child, depth - 1, alpha, beta, nodeCountRef);
                }
                int heuristic = evaluatedNode.getHeuristic();
...
//                nodeCount++;
        ...
        logPerformanceMetrics(executionTime);
    ...
    public void makeAIMove() {
        updateCurrentNode();
        int[] nodeCountRef = {0};
        Node bestNode = minimaxAB(true, currentNode, depth, Integer.MIN_VALUE, Integer.MAX_VALUE, nodeCountRef);
//        Node bestNode = minimax(true, currentNode, depth, nodeCountRef);
        System.out.println("Minimax - Tổng số node đã duyệt: " + nodeCountRef[0] + " node");
...
//				System.out.println("===========================================================");
    ...
    private void logPerformanceMetrics(long executionTime) {
        System.out.println("Minimax - Thời gian chạy: " + executionTime + " milliseconds");
